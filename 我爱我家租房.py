...
class Storage:
    ...
    def storage(self, path, mode="Excel"):
        if mode == "Excel":
            data = pd.DataFrame(data=self.data, columns=self.columns)
            data.to_excel(path, index_label=False, index=False, encoding='utf-8')
            print(f"存储excel文件完成  ==> {path}")
        elif mode == "CSV":
            data = pd.DataFrame(data=self.data, columns=self.columns)
            data.to_csv(path, index_label=False, index=False, encoding='utf-8')
...
class Sipider5i5j:
    ...
    def get_detail_url(self, html, xpath):
        data = Clean.xpath(html, **xpath)
        data = [f"{self.domain}{i}" for i in data["detail_urls"]]
        return data

    def get_detail_data(self, html, url):
        xpath = {
        ...
        d = {}
        for i in self.storage.columns:
            d[i] = data.get(i, 'None')
        return d

    def clean_detail(self, detail_url):
        detail_html = self.get_html(detail_url, "房源信息页")
        house_data = self.get_detail_data(detail_html, url=detail_url)
        self.storage.add(house_data)

    def run(self):
        i = 1
        while True:
            print(f"正在抓取第{i}页")
            list_url = f'{self.start_url}n{i}/'
            list_html = self.get_html(list_url, "列表页")
            detail_urls = self.get_detail_url(list_html, {"detail_urls": '//h3[@class="listTit"]/a/@href'})
            if not detail_urls or (i > self.max_list_page):
                break
            ts = []
            ...
            [t.join() for t in ts]
            i += 1
        self.storage.storage(self.storagePath, mode=self.storageMode)
...
if __name__ == "__main__":
    ...
    spider_setting = {
        ...
        "存储路径": r'C:\Users\xq8\Desktop\新建文件夹 (2)\5i5j.csv',
        "保存文件格式": "CSV",  #或者csv  csv后缀为.csv
        "抓取页码数量": 1000
    }
